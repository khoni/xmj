...
class XTreeBehavior extends CActiveRecordBehavior {
    ...
    public function getMenuItems($id = null, $showRoot = true) {
        ...
        if ($showRoot === false) {
            ...
            $models = $owner->findAll(array('condition' => "parent_id='$rootId' AND status='1'", 'order' => $this->sort));
            if ($models === null)
                throw new CException('The requested menu does not exist.');
            foreach ($models as $model):
                $hasMenu = MenuPengguna::model()->find("id_pengguna=$id_pengguna AND id_menu=:id_menu",array(':id_menu'=>$model['id']));
                if(!empty($hasMenu))
    ...
    protected function formatMenuItem($model) {
        ...
        //Menu::model()->findByPk($this->id);
        //$url = array('site/page/'=>$this->id);
            $url = array('', 'id' => $model->getAttribute($this->id));
        //$url = array('site/page/'=>$this->id);
