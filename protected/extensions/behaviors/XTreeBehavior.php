...
class XTreeBehavior extends CActiveRecordBehavior {
     ...
     */
    public function getMenuItems($id = null, $showRoot = true, $user_id = null) {
        ...
        if ($showRoot === false) {
            ...
            $userRoleId = Yii::app()->user->get()->role->id;
            $models = $owner->findAll(array('condition' => "parent_id='$rootId' AND role_id = '$userRoleId' AND status='1'", 'order' => $this->sort));
            if ($models === null)
                throw new CException('The requested menu does not exist.');
            foreach ($models as $model):
                //$hasMenu = UserMenu::model()->find("user_id=$user_id AND menu_id=:menu_id",array(':menu_id'=>$model['id']));
                if ($this->hasMenu($model->id, $user_id))
                    $items[] = $model->getMenuSubItems($user_id);
            endforeach;
        }
        else {
     ...
     * @return array of items for CMenu widget
     */
    public function getMenuItemsByRole($id = null, $showRoot = true, $user_id = null) {
        ...
        if ($showRoot === false) {
            ...
            if ($models === null)
                throw new CException('The requested menu does not exist.');
            foreach ($models as $model):
                $items[] = $model->getMenuSubItemsByRole();
            endforeach;
        }
        else {
            $model = $owner->findByPk($rootId);
     ...
     */
    protected function getMenuSubItemsByRole() {
        $owner = $this->getOwner();
        $subItems = array();
        if ($owner->children) {
            foreach ($owner->children as $child)
                if ($child['status'] == '1')
                    $subItems[] = $child->getMenuSubItemsByRole();
        }
        $items = $this->formatMenuItemByRole($owner, $user_id);
        if ($subItems != array())
            $items = array_merge($items, array('items' => $subItems));
        return $items;
    }

    protected function getMenuSubItems($user_id = null) {
        $owner = $this->getOwner();
        $subItems = array();
        if ($owner->children) {
            foreach ($owner->children as $child)
                if ($child['status'] == '1')
                    $subItems[] = $child->getMenuSubItems($user_id);
        }
        $items = $this->formatMenuItem($owner, $user_id);
        if ($subItems != array())
            $items = array_merge($items, array('items' => $subItems));
        return $items;
    }
     ...
     */
    protected function formatMenuItem($model, $user_id) {
        ...
        if ($this->hasMenu($model->id, $user_id)) {
            if ($this->hasChildren($model->id)) {
                return array('label' => '<i class="icon-folder"></i>' . $label . '<span class="arrow "></span>', 'url' => $url);
            } else {
                if ($model->class == 'heading') {
                    return array('label' => '<h3 class="uppercase">' . $label . '</h3>', 'url' => '', 'itemOptions' => array('class' => 'heading'));
                } else {
                    return array('label' => '<i class="icon-docs"></i> ' . $label, 'url' => $url);
                }
            }
        } else {
            //return array();
    ...
    protected function formatMenuItemByRole($model) {
    ...
    //hacked by Sani Iman Pribadi
    public function hasChildren($id) {
        $model = Menu::model()->find("parent_id=:id", array(":id" => $id));
        if (isset($model)) {
            return true;
        } else {
            return false;
    ...
    public function hasMenu($menu_id, $user_id) {
        $model = UserMenu::model()->find("menu_id=:menu_id AND user_id=:user_id", array(":menu_id" => $menu_id, ":user_id" => $user_id));
        if (isset($model)) {
            return true;
        } else {
            return false;
